...
from django.db import DEFAULT_DB_ALIAS, router, connections, transaction, connection
...
def get_existing_fields(model):
    # Get the actual table name from the model
    table_name = model._meta.db_table

    # Get a list of actual fields in the database table using introspection
    with connection.cursor() as cursor:
        ...
        existing_fields = {row[0] for row in cursor}

    valid_fields = [field for field in model._meta.fields if field.name in existing_fields];
    return valid_fields
...
def get_comments_for_model(model):
    ...
    for field in get_existing_fields(model):
        comment = []
